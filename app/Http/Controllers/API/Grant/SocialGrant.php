...
class SocialGrant extends AbstractGrant{
    ...
    {
        $provider = $this->getRequestParameter('provider_name', $request);
        if (is_null($provider)) {
    ...
    private function getUserFromSocialNetwork(Request $request){
        ...
        $socialAccount = LinkedSocialAccount::where('provider_name', $request->provider_name)->where('provider_id', $request->provider_user_id)->first();
